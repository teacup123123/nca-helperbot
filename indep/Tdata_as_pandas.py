...
from Tdata_compiled.read_dorm import read_dorm

full_source = 'Tdata_compiled\替代役訓練及管理中心役男退役管制名冊(持續更新)0312 253T.xlsx'  # incomplete
...
discharge = [x.value for x in np.array(worksheet[f'J3:J{worksheet.max_row}'])[:, 0]]
full = pd.DataFrame({'uid': uidz, 'name': namez, 'tnum': tz, 'unit': unit, 'discharge': discharge})
full = full.sort_values(by='uid').reset_index(drop=True)

start = max(uidz)
tz_treated_till = max(tz)
for fn in sorted(os.listdir('Tdata_compiled')):  # updates
    ...
    if m and int(m.group(1)) > tz_treated_till:
        ...
        for _bad_labels in read.columns:
            if _bad_labels not in ['uid', '名', '梯次', '預退日期']:
                read.drop(_bad_labels)

        read.rename(columns={"名": "name", "梯次": "tnum", '預退日期': 'discharge'}, inplace=True)
...
def fill_dorm():
    dorm = read_dorm()
    dormcolumn = []
    for name in full.name:
        dormcolumn.append(dorm.get(name, '未知'))
    return dormcolumn


full.insert(full.shape[1], 'dorm', fill_dorm())
full.to_pickle(os.path.join('Tdata_compiled', 'allT.pickle'))  # complete
