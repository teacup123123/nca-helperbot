...
# workdays = [d for d in workdays if d.month == month_repre.month]
workhours = []
for yr2 in workdays:
    for _i in range(4):
        workhours.append((yr2, 845 + 100 * _i))
    for _i in range(4):
        workhours.append((yr2, 1331 + 100 * _i))

# New year later hours removed like this TODO 特殊非全天放假
for _ in [
    (Date(2024, 2, 7), 1331 + 100 * 2),
    (Date(2024, 2, 7), 1331 + 100 * 3),
]:
    if _ in workhours: workhours.remove(_)

byworkday = collections.defaultdict(list)
for _ in workhours: byworkday[_[0]].append(_[1])
...
for _i, row in enumerate(worksheet.rows):
    ...
    # assert all(yr==113 for (yr, mn, dy), token, hrs in seen[name])
    ...
    for (yr, mn, dy), token, hrs in seen[name]:
        ...
        if starttime is not None:
            starttime, endtime = map(int, [starttime, endtime])
        ...
        if any(x in token for x in '早四 晚四 當日'.split()) or not any(x in token for x in '-~'):
            ...
            for dd in filter(lambda ddd: Date(yr, mn, dy) <= ddd <= Date(yr2, mn2, dy2), workdays):
                usage = sum(
                    ...
                    (Date(yr, mn, dy), starttime) <= (dd, x) <= (Date(yr2, mn2, dy2), endtime))
                totusage += usage
